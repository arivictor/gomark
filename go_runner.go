...
func (GoRunner) Run(ctx context.Context, code string) RunResult {
	start := time.Now()
...
func failure(start time.Time) RunResult {
	return RunResult{OK: false, Error: "cannot run", ExitCode: 1, DurationMS: time.Since(start).Milliseconds()}
}

func failureWithOutput(start time.Time, output string, code int) RunResult {
	return RunResult{OK: false, Output: output, Error: "cannot run", ExitCode: code, DurationMS: time.Since(start).Milliseconds()}
}
