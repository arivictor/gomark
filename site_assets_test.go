...
func TestAppPublicFSDefaultsToEmbeddedAssets(t *testing.T) {
	...
	for _, entry := range entries {
		if strings.EqualFold(entry.Name(), "favicon") {
			foundFavicon = true
			break
		}
	}
...
func TestAppPublicFSOverlaysOnDiskAssets(t *testing.T) {
	dir := t.TempDir()
	// favicon/favicon.ico also exists in the embedded tree (override case);
	// og-image.png does not (additive case).
	if err := os.MkdirAll(filepath.Join(dir, "favicon"), 0o755); err != nil {
		t.Fatalf("mkdir favicon: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "favicon", "favicon.ico"), []byte("custom-icon"), 0o644); err != nil {
	...
	got, err := fs.ReadFile(publicFS, filepath.Join("favicon", "favicon.ico"))
	if err != nil {
	...
	if !names["og-image.png"] || !names["favicon"] || !names["runner.wasm.gz"] {
...
func TestAppPublicFSFallsBackWhenDirMissing(t *testing.T) {
	...
	if _, err := fs.Stat(publicFS, filepath.Join("favicon", "favicon.ico")); err != nil {
...
func TestStaticFileExistsUsesFSAndBlocksTraversal(t *testing.T) {
	...
	exists, err := staticFileExists(publicFS, "/favicon/favicon.ico")
	if err != nil {
		t.Fatalf("static file exists: %v", err)
	}
	if !exists {
		t.Fatal("expected favicon/favicon.ico to exist")
	}
